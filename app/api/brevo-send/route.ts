...
export async function POST(request: Request) {
  try {
    ...
    if (emailType === 'admin_notification') {
      ...
      textContent = `New ticket ${ticketId} from ${companyName}. Customer: ${customerEmail || email}`;
      finalSubject = `[NEW TICKET] ${subject}`;
    } else if (emailType === 'regional_notification') {
      htmlContent = createRegionalNotificationTemplate({
        ticketId,
        companyName,
        contactPerson,
        subject,
      ...
      textContent = `Your support ticket ${ticketId} has been created. We will respond shortly.`;
    }
